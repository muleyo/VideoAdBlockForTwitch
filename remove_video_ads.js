              ...
              if (settingsMenu == null) {
                ...
                if (settingsCog) {
                  ...
                  if (lowQuality) {
                    ...
                    var originalQuality = JSON.parse(
                      OriginalVideoPlayerQuality
                    );
                    window.localStorage.setItem(
                      "video-quality",
                      '{"default":"' + originalQuality.group + '"}'
                    );
  ...
  fetch = async function (url, options) {
    if (typeof url === "string") {
      if (url.includes("video-weaver")) {
        ...
        if (
          url.includes("subscriber%22%3Afalse") &&
          url.includes("hide_ads%22%3Afalse") &&
          url.includes("show_ads%22%3Atrue")
        ) {
          useNewUsher = true;
        }
        if (
          url.includes("subscriber%22%3Atrue") &&
          url.includes("hide_ads%22%3Afalse") &&
          url.includes("show_ads%22%3Atrue")
        ) {
          useNewUsher = true;
        ...
        if (useNewUsher == true) {
          return new Promise(function (resolve, reject) {
            var processAfter = async function (response) {
              encodingsM3u8 = await getNewUsher(
                realFetch,
                response,
                channelName
              );
    ...
    if (accessTokenResponse.status === 200) {
      ...
      try {
        ...
        if (encodingsM3u8Response.status === 200) {
          ...
          streamM3u8Url = encodingsM3u8.match(/^https:.*\.m3u8$/gm)[0];
